package com.javarush.games.minesweeper;

import com.javarush.engine.cell.Color;
import com.javarush.engine.cell.Game;

import java.util.ArrayList;
import java.util.List;

public class MinesweeperGame extends Game {
    private static final int SIDE = 9;
    private GameObject[][] gameField = new GameObject[SIDE][SIDE];
    private int countMinesOnField;
    private final static String MINE = "\uD83D\uDCA3";
    private final static String FLAG = "\uD83D\uDEA9";
    private int countFlags;
    private boolean isGameStopped;
    private int countClosedTiles = SIDE * SIDE;
    private int score;

    @Override
    public void initialize() {
        setScreenSize(SIDE, SIDE);
        createGame();
    }

    private void createGame() {

        for (int y = 0; y < SIDE; y++) {
            for (int x = 0; x < SIDE; x++) {
                boolean isMine = getRandomNumber(10) < 2;
                if (isMine) {
                    countMinesOnField++;
                }
                gameField[y][x] = new GameObject(x, y, isMine);
                setCellColor(x, y, Color.ORANGE);
                setCellValue(x, y, "");
            }
        }
        countFlags = countMinesOnField;
        countMineNeighbors();
    }

    private List<GameObject> getNeighbors(GameObject gameObject) {
        List<GameObject> result = new ArrayList<>();
        for (int y = gameObject.y - 1; y <= gameObject.y + 1; y++) {
            for (int x = gameObject.x - 1; x <= gameObject.x + 1; x++) {
                if (y < 0 || y >= SIDE) {
                    continue;
                }
                if (x < 0 || x >= SIDE) {
                    continue;
                }
                if (gameField[y][x] == gameObject) {
                    continue;
                }
                result.add(gameField[y][x]);
            }
        }
        return result;
    }

    private void countMineNeighbors() {
        for (int y = 0; y < SIDE; y++) {
            for (int x = 0; x < SIDE; x++) {
                GameObject gameObject = gameField[y][x];

                if (gameObject.isMine == false) {
                    List<GameObject> neighbors = getNeighbors(gameObject);
                    for (GameObject neighbor : neighbors) {
                        if (neighbor.isMine == true) {
                            gameObject.countMineNeighbors++;
                        }
                    }

                }

            }

        }

    }

    private void openTile(int x, int y) {
        if (isGameStopped == false) {
            GameObject object = gameField[y][x];
            if (object.isOpen == false) {
                if (object.isFlag == false) {
                    object.isOpen = true;
                    countClosedTiles--;
                    gameField[y][x] = object;
                    if (object.isMine == true) {
                        setCellValueEx(x, y, Color.RED, MINE);
                        gameOver();

                    } else {
                        score = score + 5;
                        setScore(score);
                        if (countClosedTiles == countMinesOnField) {
                            setCellColor(x, y, Color.GREEN);
                            win();
                        } else {
                            if (object.countMineNeighbors == 0) {
                                setCellColor(x, y, Color.GREEN);
                                setCellValue(x, y, "");
                                List<GameObject> neighbors = getNeighbors(object);
                                for (GameObject neighbor : neighbors) {
                                    if (neighbor.isOpen == false) {
                                        openTile(neighbor.x, neighbor.y);
                                    }
                                }

                            } else {
                                setCellNumber(x, y, object.countMineNeighbors);
                                setCellColor(x, y, Color.GREEN);
                            }
                        }
                    }

                    //setCellColor(x, y, Color.GREEN);
                }
            }
        }

    }

    @Override
    public void onMouseLeftClick(int x, int y) {
        if (isGameStopped == true) {
            restart();
        } else {
            openTile(x, y);
        }

    }

    private void markTile(int x, int y) {
        if (isGameStopped == false) {
            GameObject object = gameField[y][x];
            if (object.isOpen == false) {
                if (object.isFlag == false && countFlags != 0) {
                    object.isFlag = true;
                    setCellValue(x, y, FLAG);
                    setCellColor(x, y, Color.YELLOW);
                    countFlags--;
                } else if (object.isFlag == true) {
                    object.isFlag = false;
                    countFlags++;
                    setCellValue(x, y, "");
                    setCellColor(x, y, Color.ORANGE);
                }

            }
            gameField[y][x] = object;
        }
    }

    @Override
    public void onMouseRightClick(int x, int y) {
        markTile(x, y);
    }

    private void gameOver() {
        isGameStopped = true;
        showMessageDialog(Color.BLACK, "GAME OVER! To restart click the LEFT mouse button", Color.RED, 20);

        for (int y = 0; y < SIDE; y++) {
            for (int x = 0; x < SIDE; x++) {
                GameObject gameObject = gameField[y][x];
                if (gameObject.isMine == true) {
                    setCellValueEx(x, y, Color.RED, MINE);
                }
            }
        }
    }

    private void win() {
        isGameStopped = true;
        showMessageDialog(Color.BLACK, "VICTORY!!!", Color.RED, 70);

    }

    private void restart() {
        isGameStopped = false;
        countClosedTiles = SIDE * SIDE;
        score = 0;
        countMinesOnField = 0;
        setScore(score);
        createGame();

    }
}